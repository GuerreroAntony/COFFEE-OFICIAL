...
class CanvasESPMScraper:
    ...
    async def scrape_course_materials(
        ...
        # Fallback: if no "Aula" links found, grab all file-like module items
        is_fallback = False
        if not aula_links:
            is_fallback = True
            logger.info(
                "Nenhum 'Aula' encontrado no curso %d — fallback: todos os arquivos.",
                course_id,
            )
            aula_links = await page.evaluate("""
                () => {
                    ...
                    const excludeTypes = [
                        ...
                        'ExternalUrl', 'ExternalTool', 'ContextModuleSubHeader'
                    ];
                    links.forEach(a => {
                        const text = a.textContent?.trim();
                        if (!text) return;
                        const item = a.closest('.context_module_item');
                        if (item) {
                            const classes = item.className || '';
                            const isExcluded = excludeTypes.some(t =>
                                classes.includes(t) || classes.includes(t.toLowerCase())
                            );
            ...
            """)
            logger.info(
                "Fallback: encontrados %d links no curso %d.",
                len(aula_links), course_id,
            )
            ...
            try:
                ...
                if download_link:
                    ...
                    if download_href:
                        ...
                        # In fallback mode, skip non-PDF files before downloading
                        if is_fallback:
                            # Check URL and page for PDF indicators
                            is_pdf = dl_url.lower().endswith(".pdf") or "/preview?annotate" in dl_url
                            if not is_pdf:
                                # Check file name shown on page
                                file_label = await page.evaluate("""
                                    () => {
                                        const el = document.querySelector(
                                            '.display_name, .filename, h2, .file-header .name'
                                        );
                            ...
                            if not is_pdf:
                                logger.info("    ⏭️  Ignorado (não é PDF): %s", name[:60])
                                continue
