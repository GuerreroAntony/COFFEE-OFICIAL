...
async def espm_login(
    ...
    await db.execute(
        "UPDATE users SET encrypted_portal_session = $1, espm_login = $2 WHERE id = $3",
        encrypted, body.login, user_id
    )
...
async def _upsert_disciplinas(db, user_id: UUID, disciplines: list[dict]) -> int:
    ...
    for disc in disciplines:
        try:
            # Upsert disciplina (dedup por nome + semestre)
            disc_id = await db.fetchval(
                """
                INSERT INTO disciplinas (nome, professor, horario, semestre, codigo_espm)
                VALUES ($1, $2, $3, $4, $5)
